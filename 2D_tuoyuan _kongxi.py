...
fields=(('Please input the length of the test piece(mm):','100'),#0
        ...
        ('Please enter the maximum number of cycles (integer):','800000'),#9
        ('Please input the ratio of b(long axis) to a(short axis):', '1.42'),# 10
        ('Please enter the pore ratio:', '0.05'))  # 11
...
S_kongxi=length*width*float(modelparam[11])
...
fit_ratio=float(modelparam[10])

def intercheck(point,center):
    for p in center:
        if sqrt((p[0]-point[0])**2+(p[1]-point[1])**2)<=1.05*(point[2]+p[2]):
            sign=False
            break
        else:
            sign=True
...
dgl_param=[]
S_already=0
S_zuixiaogl=round(0.25*gl_sec*gl_sec/fit_ratio*math.pi,2)
...
while S_max - S_already >= S_zuixiaogl:
    ...
    if len(all_yuan)==0:
        dx1 = x_gl+r_gl*math.cos(angle1)
        dy1 = y_gl+r_gl*math.sin(angle1)
        dx2 = x_gl+duanzhou*math.sin(angle2)
        dy2 = y_gl+duanzhou*math.cos(angle2)
        point.append([dx1,dy1,dx2,dy2])

        all_yuan.append(point)
        dgl_param.append(point)

        S_already = S_already + round(math.pi * r_gl * duanzhou, 2)
...
zgl_param=[]
S_already=0
S_zuixiaogl=round(0.25*gl_thr*gl_thr/fit_ratio*math.pi,2)
...
xgl_param=[]
S_already=0
S_zuixiaogl=round(0.25*gl_min*gl_min/fit_ratio*math.pi,2)
...
print('xgl_num:',len(kongxi_param))
...
for p in dgl_param+zgl_param+xgl_param:
    x, y,r,dx1,dy1,dx2,dy2= p[0], p[1], p[2],p[3][0],p[3][1],p[3][2],p[3][3]
    mySketch.EllipseByCenterPerimeter(center=(x, y), axisPoint1=(dx1, dy1),axisPoint2=(dx2, dy2))
    myPart.PartitionFaceBySketch(faces=f[:], sketch=mySketch)

for p in kongxi_param:
    x, y,r= p[0], p[1], p[2]
...
for  k in kongxi_param:
    ...
    face=f.findAt((x,y,z),)
    a=f.index(face)
    myPart.RemoveFaces(faceList=f[a:a+1], deleteCells=False)
